...
export default function CoordinateurSessionsPage() {
  ...
  useEffect(() => {
    ...
    const fetchSessions = async () => {
      try {
        setLoading(true)
        const res = await fetch('/api/coordinateur/sessions')
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-20 bg-white/80 rounded-2xl shadow-inner border border-gray-200/60">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-500 mb-6">Vous ne coordonnez aucune session pour l'instant.</p>
          </div>
        </div>
      ) : (
              ...
              <div className="p-6 flex flex-col gap-4 h-full">
                <div className="flex items-center gap-2 text-sm text-gray-600 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    <strong className="text-gray-900">
                      {new Date(sessionItem.dateDebut).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }).replace('.', '')}
                    </strong>
                    <span className="font-medium">au</span>
                    <strong className="text-gray-900">
                      {new Date(sessionItem.dateFin).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }).replace('.', '')}
                    </strong>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="p-3 bg-blue-50/50 rounded-xl border border-blue-100">
                    ...
                    <p className="text-sm text-gray-900 font-semibold">{sessionItem.coordinateur?.name || 'Non attribué'}</p>
                  </div>
                  ...
                  <div className="p-3 bg-orange-50/50 rounded-xl border border-orange-100">
                    ...
                    <p className="text-sm text-gray-900 font-semibold line-clamp-2">
                      {Array.isArray(sessionItem.formateurs) && sessionItem.formateurs.length > 0
                        ? sessionItem.formateurs.map((f) => f.name).join(', ')
                        : 'Non attribué'}
                    </p>
                  </div>
                </div>
