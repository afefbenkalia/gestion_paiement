...
export default function CoordinateurFichePage() {
  ...
  useEffect(() => {
    ...
    const fetchFiche = async () => {
      try {
        setLoading(true)
        const res = await fetch(`/api/coordinateur/fiches/${sessionId}`)
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
        <div className="w-11/12 mx-auto px-4">
          ...
          <div className="mt-4">
            <button onClick={() => router.push('/coordinateur/historique')} className="px-4 py-2 bg-gray-100 rounded-lg">Retour</button>
          </div>
        </div>
      </main>
    </div>
