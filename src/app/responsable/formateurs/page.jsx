...
export default function FormateursPage() {
  ...
  const fetchFormateurs = async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams({ role: 'FORMATEUR', status: 'ACTIVE' });
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Liste des formateurs actifs ({formateurs.length})</p>
      </div>

      {/* Cards Grid */}
      {formateurs.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
          Aucun formateur actif trouvé
        </div>
      ) : (
            ...
            >
              <div className="bg-linear-to-r from-blue-500 to-blue-600 px-6 py-4">
                <div className="flex items-start justify-between">
                  ...
                  <span className="px-2 py-1 text-xs font-semibold rounded-full bg-white bg-opacity-20 text-blue-500">
                    {getTimeSinceCreation(formateur.createdAt)}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <span>{formateur.tel || 'Non renseigné'}</span>
                </div>

                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <BookOpen className="h-4 w-4 text-gray-400 shrink-0" />
                  <span className="font-medium text-blue-600">
                    {formateur.sessionCount} session{formateur.sessionCount > 1 ? 's' : ''} assignée{formateur.sessionCount > 1 ? 's' : ''}
                  </span>
                </div>
              </div>
