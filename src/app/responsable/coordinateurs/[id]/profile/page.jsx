...
export default function CoordinateurProfileManagePage() {
  const params = useParams()
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [msg, setMsg] = useState(null)
  const [user, setUser] = useState(null)
  const [form, setForm] = useState({ rib: '', banque: '', fonction: '' })
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setUser(data)
        setForm({ rib: data.rib || '', banque: data.banque || '', fonction: data.fonction || '' })
      } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/responsable/users/${params.id}`, {
        ...
        body: JSON.stringify({ rib: form.rib || null, banque: form.banque || null, fonction: form.fonction || null }),
      })
  ...
  if (!user) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen">
        ...
        <Link href={`/responsable/coordinateurs/${params.id}`} className="text-blue-600 hover:underline">
          Retour au profil
        </Link>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-linear-to-r from-blue-600 to-blue-700 px-6 py-8 text-white">
            ...
            <p className="text-blue-100 mt-2">Gérer les informations financières et fonction du coordinateur</p>
          </div>
              ...
              <div className="lg:col-span-2">
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-end pt-4">
                    ...
                    <Link href={`/responsable/coordinateurs/${params.id}`} className="ml-3 text-gray-600 hover:text-gray-800">
                      Annuler
                    </Link>
                  </div>
                </form>
